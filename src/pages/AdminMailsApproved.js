...
export const Blogs = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      ...
      try {
        const response = await axios.get(`http://172.235.21.99:9591/blog/retrieveapprovedmails/nothing`,{
            headers: {
  ...
  const handleapprovePost = async (value) => {
    const accessToken = localStorage.getItem("accessToken");
    try {
      await axios.post(`http://172.235.21.99:9591/blog/revokemail/${value}`, {}, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      });
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", width: "calc(100vw - 260px)" }}>
      <div style={{ width: "100%" }}>
        <div style={{ width: "100%", display: "flex", justifyContent: "center", fontSize: '28px', marginBottom: "20px" }}>Approved Mails</div>
            ...
            <CardFooter >
              <div style={{ display: "flex", width: "100%", justifyContent: "space-between" }}>
                <div style={{ display: "flex", width: "16%", justifyContent: "space-between" }}>
                  <Button icon={<EditFilled fontSize={16} />} onClick={() => handleapprovePost(blog.postId)}>Revoke</Button>
