...
export const Blogs = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      ...
      try {
        const response = await axios.get(`http://172.235.21.99:9591/blog/retrieveapprovedposts/nothing`,{
            headers: {
  ...
  const handleapprovePost = async (value) => {
    const accessToken = localStorage.getItem("accessToken");
    try {
      await axios.post(`http://172.235.21.99:9591/blog/revokepost/${value}`, {}, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      });
