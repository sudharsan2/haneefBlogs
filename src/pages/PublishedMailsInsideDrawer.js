...
export const Blogs = ({ empId }) => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      ...
      try {
        const response = await axios.get(`http://172.235.21.99:9591/blog/retrievepublishedmails/${empId}`, {
          headers: {
  ...
  const handleapprovePost = async () => {
    const accessToken = localStorage.getItem("accessToken");
    try {
      await axios.post(`http://172.235.21.99:9591/blog/revokemail/${selectedPostId}`, {}, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      });
