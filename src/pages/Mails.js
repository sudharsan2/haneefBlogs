...
export const Blogs = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      ...
      try {
        const response = await axios.get('http://172.235.21.99:9591/blog/retrievemails',{
            headers: {
  ...
  const handleDelete = async (value) => {
    const accessToken = localStorage.getItem("accessToken");
  try {
    const response = await axios.post('http://172.235.21.99:9591/blog/deletemail',{"postId":value},{
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      });
...
const handlePublish = async (value) => {
    const accessToken = localStorage.getItem("accessToken");
  try {
    const response = await axios.post('http://172.235.21.99:9591/blog/publishmail',{"postId":value},{
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      });
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", width: "calc(100vw - 260px)" }}>
      <div style={{ width: "100%" }}>
        <div style={{ width: "90%", display: "flex", justifyContent: "center", fontSize: '28px', marginBottom: "20px" }}>Posted Mails</div>
