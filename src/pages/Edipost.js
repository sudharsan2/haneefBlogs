...
export default function Edit({contents}) {
  const styles = useStyles();
  const [title, setTitle] = useState(contents.title);
  const [content, setContent] = useState("");
  const [author, setAuthor] = useState(contents.footer);
  const [fileList, setFileList] = useState([]);
  const [postId, setPostId] = useState(contents.postId);
  const [editorValue, setEditorValue] = useState(contents.content);
  const [isMultiline, setIsMultiline] = useState(true);
  const [currentTab, setCurrentTab] = useState(tabs[2].value);
  const [base64, setBase64] = useState('');
  const [footer, setFooter] = useState(contents.author);
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", overflowX: "scroll", width: "calc(100vw - 260px)" }}>
